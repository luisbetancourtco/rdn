...
export default async function AsesoriasLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className={`flex-1 bg-md-surface min-w-0 pt-14 md:pt-0 ${session.impersonating ? 'mt-10' : ''}`}>{children}</main>
    </div>
  )
}
